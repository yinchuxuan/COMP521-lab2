...
int LoadProgram(char *name, char **args) {
    ...
    int vpn;
    struct pte* page_table = current_pcb->region0_page_table;
    ...
    for (address = VMEM_BASE; address < VMEM_0_LIMIT; address += PAGESIZE) {
        ...
        if (address < DOWN_TO_PAGE(KERNEL_STACK_BASE) || address >= DOWN_TO_PAGE(KERNEL_STACK_LIMIT)) {
            if (page_table[vpn].valid) {    // should free physical page and ummap virtual page
                struct physical_page_frame* page_frame = physical_address_to_page_frame(page_table[vpn].pfn << PAGESHIFT);
                ...
                page_table[vpn].valid = INVALID_PAGE;
                page_table[vpn].pfn = 0;
                page_table[vpn].kprot = 0;
                page_table[vpn].uprot = 0;
            }
        } 
    }
     ...
     *       pfn   = a new page of physical memory
     */
    for (vpn = MEM_INVALID_PAGES + text_npg; vpn < MEM_INVALID_PAGES + text_npg + data_bss_npg; vpn++) {
        struct physical_page_frame* page_frame = allocate_page();
     ...
     */
    for (vpn = MEM_INVALID_PAGES; vpn < MEM_INVALID_PAGES + text_npg; vpn++) {
        page_table[vpn].kprot = PAGE_READ | PAGE_EXECUTE;
    }
